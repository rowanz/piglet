...
class Encoder:
    def __init__(self, encoder, bpe_merges, errors='replace', add_space=True):
    ...
    def decode_list(self, tokens):
        """Decode tokens into a list such that if we were to call ''.join(x) on it, it would give us the original thing"""

        return [bytearray([self.byte_decoder[c] for c in self.decoder[t]]).decode('utf-8', errors=self.errors) for t in tokens]
...
def random_sliding_window(tokens, max_seq_length):
    """
    Randomly sample some spans of a longer article
    :param tokens:
